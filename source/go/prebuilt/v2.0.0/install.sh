...
# Claude Code starts, NOT only in settings.json. Claude Code's own process
# reads its own env at startup to decide Bedrock vs API mode; settings.json
# is applied later and only to child processes Claude Code itself spawns.
# If we rely on settings.json alone, Claude Code boots in API mode (the
# "Sonnet 4.5 · API Usage Billing" banner), and the wrapper's
# ANTHROPIC_MODEL is ignored because Claude Code isn't asking the Bedrock
# SDK for anything.
#
# The variable-assignment-before-command syntax scopes all four vars to
# just this single invocation -- the user's shell is not polluted.
print('  CLAUDE_CODE_USE_BEDROCK=1 \\')
print('  AWS_REGION=' + aws_region + ' \\')
print('  AWS_PROFILE=' + prof_name + ' \\')
print('  ANTHROPIC_MODEL="$arn" \\')
